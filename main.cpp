...
int main() {
    ...
    locations.emplace_back(Point<ndim>(0, {.1, .1}));
    locations.emplace_back(Point<ndim>(1, {.3, .3}));
    locations.emplace_back(Point<ndim>(2, {.1, .1}));
    locations.emplace_back(Point<ndim>(3, {.5, .1}));
    locations.emplace_back(Point<ndim>(4, {.5, .1}));
    locations.emplace_back(Point<ndim>(5, {.1, .1}));
    locations.emplace_back(Point<ndim>(6, {.1, .1}));
    locations.emplace_back(Point<ndim>(7, {.3, .3}));
    locations.emplace_back(Point<ndim>(8, {.3, .3}));
    locations.emplace_back(Point<ndim>(9, {.1, .1}));

    std::vector<Real> data_time{0.75, 0.25, 0.35, 0.15, 0.75, 0.65, 0.25, 0.35, 0.75, 0.95}; //!***

    std::vector<Real> lambda{0.01 , 0.1 , 1.}; //!***
    std::vector<Real> stepProposals{0.1, 1., 10., 100.}; //!***
    VectorXr fvec = VectorXr ::Ones(481); //!*** 481
    fvec = 10*fvec;
    ...
    //!#################################################################################################################
    //!################################# BEGINNING OF THE DE CODE ######################################################
    //!#################################################################################################################

    DEData_time<ndim> dedatatime(locations, data_time);
    DataProblem_time<ORDER, mydim, ndim> dataProblem_time(locations, data_time, 2, fvec, 0.023, 100, lambda, 7, 10, stepProposals,
                                  0.0001, 0., false, 1, points_m, sides_m, elements_m, neighbors_m, time_mesh);
    ...
    //std::cout<<"User gInit size: "<<g.size()<<std::endl;

    //std::tuple<Real, VectorXr, Real, Real, Real> result = functionalProblem_time.computeFunctional_g(g,0.5,0.5,dataProblem_time.getUpsilon());

    //std::tuple<Real, Real, Real> result_f = functionalProblem_time.computeLlikPen_f(g);


    std::shared_ptr<MinimizationAlgorithm_time<ORDER, mydim, ndim>> minimizationAlgo_time =
            MinimizationAlgorithm_factory_time<ORDER, mydim, ndim>::createStepSolver(dataProblem_time, functionalProblem_time, direction_method, step_method);

    //Construct FEDE object
    FEDE_time<ORDER, mydim, ndim> fede(dataProblem_time, functionalProblem_time, minimizationAlgo_time, preprocess_method);
    ...
    for(UInt j=0; j < g_sol.size(); j++)
        std::cout<<j << "-th row: " << g_sol[j]<<std::endl;
