...
#ifndef DEV_FDAPDE_DENSITY_INITIALIZATION_IMP_H
#define DEV_FDAPDE_DENSITY_INITIALIZATION_IMP_H
...
HeatProcess<ORDER, mydim, ndim>::computeStartingDensities(){
    ...
    // it saves in i-th position the set of the neighbour nodes id that have node i
...
template<UInt ORDER, UInt mydim, UInt ndim>
const VectorXr*
UserInitialization_time<ORDER, mydim, ndim>::chooseInitialization(Real lambda_S, Real lambda_T) const{
...
HeatProcess_time<ORDER, mydim, ndim>::HeatProcess_time(const DataProblem_time<ORDER, mydim, ndim>& dp,
    ...
    //std::cout<<"INTO HEAT PROCESS CONSTRUCTOR"<<std::endl;
...
template<UInt ORDER, UInt mydim, UInt ndim>
void
HeatProcess_time<ORDER, mydim, ndim>::computePatchAreas(){
...
template<UInt ORDER, UInt mydim, UInt ndim>
VectorXr
HeatProcess_time<ORDER, mydim, ndim>::computeDensityOnlyData(UInt num_basis){
...
template<UInt ORDER, UInt mydim, UInt ndim>
void
HeatProcess_time<ORDER, mydim, ndim>::computeStartingDensities(){
    ...
    for(UInt j = 0; j < niter_; ++j){
        ...
        //std::cout<<"init proposals "<<std::endl;
        init_proposals_[j] = x_new.array() + epsilon_;  // modify initial density
        //std::cout<<"init proposals end"<<std::endl;
...
template<UInt ORDER, UInt mydim, UInt ndim>
const VectorXr*
HeatProcess_time<ORDER, mydim, ndim>::chooseInitialization(Real lambda_S, Real lambda_T) const{
    ...
    //Rprintf("The initialization selected for lambda %f is the number %d\n", lambda, index_min);
    std::cout << "The initialization selected for lambda_S " << lambda_S << " and lambda_T " << lambda_T << " is the number: " << index_min << std::endl; //!added for c++ purpose

    VectorXr g_heat = init_proposals_[index_min].array().log();
    VectorXr result = createSTSolution(this->dataProblem_.getNumNodes(), this->dataProblem_.getSplineNumber(), this->dataProblem_.getMesh_time(), this->dataProblem_.getMesh_time(), g_heat);
    writeSolution<VectorXr>(result.array().exp(), "../data/space_time/heat_choice.txt");
...
template<UInt ORDER, UInt mydim, UInt ndim>
void
Heat_CV_time<ORDER, mydim, ndim>::perform_init_cv(){
