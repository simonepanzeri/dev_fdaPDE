...
#ifndef DEV_FDAPDE_PREPROCESS_PHASE_IMP_H
#define DEV_FDAPDE_PREPROCESS_PHASE_IMP_H
...
CrossValidation<ORDER, mydim, ndim>::performCV(){
    ...
    for (UInt i = 0; i < K; i++){
        ...
        if(this->dataProblem_.Print()){
            std::cout << "X_valid is the fold number " << i << std::endl;
...
template<UInt ORDER, UInt mydim, UInt ndim>
std::tuple<VectorXr, Real, Real>
CrossValidation_time<ORDER, mydim, ndim>::performCV(){
...
/*
        ...
        std::cout << "x_valid: " << x_valid.size() << std::endl;
        for (UInt k : x_valid)
            std::cout << k << " ";
        std::cout << std::endl;
*/
    ...
    //std::cout << "best lambda_S: " << this->dataProblem_.getLambda(init_best_lambda_S) << ", best lambda_T: " << this->dataProblem_.getLambda_time(init_best_lambda_T) << std::endl;
...
template<UInt ORDER, UInt mydim, UInt ndim>
void
CrossValidation_time<ORDER, mydim, ndim>::performPreprocessTask(){
...
template<UInt ORDER, UInt mydim, UInt ndim>
void
SimplifiedCrossValidation_time<ORDER, mydim, ndim>::performCV_core(UInt fold, const SpMat& Upsilon_train, const SpMat& Upsilon_valid){
    ...
    if(this->dataProblem_.Print()){
        std::cout << "lambda_S: " << this->dataProblem_.getLambda(fold_S) << std::endl;
        std::cout << "lambda_T: " << this->dataProblem_.getLambda_time(fold_T) << std::endl;
        //Rprintf("lambda_S, lambda_T: %f, %f\n", this->dataProblem_.getLambda(fold_S), this->dataProblem_.getLambda_time(fold_T));
    }
...
template<UInt ORDER, UInt mydim, UInt ndim>
RightCrossValidation_time<ORDER, mydim, ndim>::
...
template<UInt ORDER, UInt mydim, UInt ndim>
void
RightCrossValidation_time<ORDER, mydim, ndim>::performCV_core(UInt fold, const SpMat& Upsilon_train, const SpMat& Upsilon_valid){
    ...
    for (UInt l = 0; l < this->dataProblem_.getNlambda() * this->dataProblem_.getNlambda_time(); ++l){
        ...
        if(this->dataProblem_.Print()){
            std::cout << "lambda_S: " << this->dataProblem_.getLambda(l_S) << std::endl;
            std::cout << "lambda_T: " << this->dataProblem_.getLambda_time(l_T) << std::endl;
            //Rprintf("lambda_S, lambda_T: %f, %f\n", this->dataProblem_.getLambda(fold_S), this->dataProblem_.getLambda_time(fold_T));
        }
